...
$products = [];
// Perhatikan: Menggunakan 'nama', 'keterangan', 'stok', dan 'image' sesuai struktur tabel 'produk' Anda
...
<head>
    ...
    <style>
        ...
        .product-card .out-of-stock {
            color: red;
            font-weight: bold;
            display: none; /* Hidden by default, shown if stock is 0 */
        }
...
<body>
    ...
    <script>
        let cart = {}; // Stores {productId: {id, name, price, quantity, stock}}
        ...
        function updateQuantity(productId, change) {
            ...
            // Mencegah penambahan melebihi stok yang tersedia
            if (change > 0 && newQty > currentStock) {
                alert('Jumlah pesanan melebihi stok yang tersedia (' + currentStock + ').');
                newQty = currentStock; // Atur kuantitas maksimal ke stok yang ada
            }
            ...
            if (newQty > 0) {
                cart[productId] = {
                    ...
                    price: productPrice,
                    quantity: newQty,
                    stock: currentStock // Simpan stok asli untuk referensi
                };
            } else {
        ...
        async function placeOrder() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // Update tampilan stok dan nonaktifkan tombol jika stok menjadi 0
                    if (result.updated_stocks) {
                        result.updated_stocks.forEach(item => {
                            ...
                            if (stockSpan) {
                                ...
                                if (productCard) {
                                    // Update data-stock attribute on the card
                                    productCard.dataset.stock = item.new_stock; 
                                    const plusButton = productCard.querySelector('.quantity-controls button:last-child');
                                    if (item.new_stock <= 0) {
                                        if (plusButton) plusButton.disabled = true; // Nonaktifkan tombol '+'
                                        if (oosMessage) oosMessage.style.display = 'block'; // Tampilkan "Stok Habis"
                                    } else {
                                        if (plusButton) plusButton.disabled = false; // Aktifkan lagi jika stok > 0
                                        if (oosMessage) oosMessage.style.display = 'none'; // Sembunyikan "Stok Habis"
                                    }
                                }
                            }
                        });
